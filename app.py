...
if not st.session_state.get("logged_in", False):
    ...
    <style>
        ...
        .auth-card { background: rgba(255, 255, 255, 0.98); border-radius: 30px; padding: 26px 30px 30px; height: var(--hero-height); min-height: var(--hero-min); width: 100%; box-shadow: 0 26px 70px rgba(0,0,0,0.18); display: flex; flex-direction: column; }
        .auth-card div[data-testid="stTabs"] { height: 100%; display: flex; flex-direction: column; }
        .auth-card div[data-testid="stTabs"] [data-baseweb="tab-list"] { align-self: flex-start; background: #eef2ff; padding: 6px; border-radius: 999px; margin-bottom: 16px; gap: 6px; box-shadow: inset 0 0 0 1px rgba(59,130,246,0.18); }
        .auth-card div[data-testid="stTabs"] [data-baseweb="tab"] { border-radius: 999px; padding: 8px 18px; font-weight: 700; color: #475569; background: transparent; }
        .auth-card div[data-testid="stTabs"] [data-baseweb="tab"][aria-selected="true"] { background: #1e3a8a; color: #ffffff; box-shadow: 0 10px 20px rgba(15,23,42,0.18); }
        .auth-card div[data-testid="stTabs"] [data-baseweb="tab-panel"] { height: calc(100% - 56px); overflow: auto; }
        .auth-card div[data-testid="stForm"] { background: transparent; border-radius: 0; padding: 0; border: none; width: 100%; height: auto; min-height: 0; max-height: none; overflow: visible; box-shadow: none; display: flex; flex-direction: column; justify-content: flex-start; }
        ...
        .auth-card div[data-testid="stForm"] label { font-size: 0.85rem; font-weight: 600; color: #475569; }
        .auth-card div[data-testid="stForm"] input, .auth-card div[data-testid="stForm"] select, .auth-card div[data-testid="stForm"] div[data-baseweb="select"] > div { background-color: #f8fafc !important; border: 1px solid #e2e8f0 !important; border-radius: 12px !important; color: #334155 !important; height: 48px; }
        .auth-card div[data-testid="stForm"] input:focus { border-color: #3b82f6 !important; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1) !important; }
        .auth-card div[data-testid="stForm"] button { background: linear-gradient(90deg, #22c55e 0%, #16a34a 100%); color: white; border: none; border-radius: 12px; font-weight: 700; padding: 0.75rem 1rem; width: 100%; font-size: 1rem; margin-top: 10px; transition: 0.3s ease; }
        .auth-card div[data-testid="stForm"] button:hover { transform: scale(1.02); box-shadow: 0 5px 15px rgba(34, 197, 94, 0.4); }
    </style>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
