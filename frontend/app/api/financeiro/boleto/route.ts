...
export async function GET(req: NextRequest) {
  ...
  // Prioridade 3: exibir página com linha digitável e Pix copia-e-cola
  ...
  const linhaSection = linhaDigitavel
    ? `<div class="bar">${linhaDigitavel}</div><p style="text-align:center;font-size:12px;color:#64748b">Linha digitável — copie e pague em qualquer banco</p>`
  ...
  const aviso = !boletoUrl && !linhaDigitavel && !pixQrCode
    ? `<div style="margin-top:16px;padding:12px;background:#fef9c3;border:1px solid #fde047;border-radius:6px;font-size:13px">⚠️ O boleto bancário ainda não foi gerado. Entre em contato para solicitar a segunda via.</div>`
    : "";
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Boleto ${codigo}</title><style>
    ...
    ${pixSection}
    ${aviso}
    <p style="margin-top:20px;font-size:12px;color:#64748b">Documento gerado pelo sistema Ativo Educacional. Use a opção imprimir do navegador para salvar em PDF.</p>
