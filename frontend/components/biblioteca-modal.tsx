...
/* ─── Modal Material / Apostila ─── */
function MaterialModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ titulo: "", url: "", turma: "", tipo: "Apostila", descricao: "" });
  const [saving, setSaving] = useState(false);
  const [erro, setErro] = useState("");
  ...
  async function salvar() {
    if (!form.titulo.trim()) { setErro("O título é obrigatório."); return; }
    setSaving(true);
    const res = await fetch("/api/biblioteca?tipo=materiais", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form),
    });
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div className="modal-header">
          <div className="modal-title">Adicionar material / apostila</div>
          <button className="modal-close" onClick={onClose}>
            <svg viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
          </button>
        </div>
        <div className="modal-body">
          <div className="form-grid">
            <div className="form-group form-group-span2">
              <label className="form-label">Título *</label>
              <input className="form-input" placeholder="Ex: Apostila Gramática Nível A1" value={form.titulo} onChange={(e) => update("titulo", e.target.value)} autoFocus />
            </div>
            <div className="form-group">
            ...
            <div className="form-group form-group-span2">
              <label className="form-label">Link / URL do arquivo</label>
              <input className="form-input" type="url" placeholder="https://..." value={form.url} onChange={(e) => update("url", e.target.value)} />
              <span className="form-hint">Cole o link do Google Drive, Dropbox ou servidor</span>
            </div>
            <div className="form-group form-group-span2">
        ...
        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={onClose} disabled={saving}>Cancelar</button>
          <button className="btn btn-primary" onClick={salvar} disabled={saving}>{saving ? "Salvando…" : "Adicionar material"}</button>
        </div>
      </div>
    </div>
  );
...
export function AdicionarMaterialBtn() {
  ...
  return (
    <>
      ...
      {open && <MaterialModal onClose={() => setOpen(false)} onSaved={() => { setOpen(false); router.refresh(); }} />}
    </>
  );
}
